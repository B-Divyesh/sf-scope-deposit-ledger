# Repair handoff — Scope Deposit Ledger 1.0.3
...
- The license dialog now checks the configured checkout endpoint only after an
  operator intentionally opens license options.
...
- The PWA release stamp/cache was advanced from `1.0.2` to `1.0.3` so an
  installed client receives the updated shell.

Exact regression coverage was added in `tests/e2e/ledger.spec.ts`: it
intercepts the verifier's exact 404 JSON body and asserts that no `Buy
unlimited` control or checkout anchor is present, while restore remains
available.
...
worker control, active cache/manifest release `1.0.3`, direct legal routes,
...
`dist/index.html` is at the static output root. Build sizes: main JS 28,799 B
(9.86 KB gzip), CSS 17,285 B (4.71 KB gzip), self-hosted fonts 102,036 B total.
...
the release tests. Normal first load makes no billing request; checkout is
contacted only after the operator opens license options.
...
fresh browser profile. The repaired UI will automatically reveal the purchase
action only once that endpoint is available.
